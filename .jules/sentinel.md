...
## 2024-06-15 - [Mitigate Server-Side Request Forgery (SSRF) in Media Downloads]
**Vulnerability:** The WhatsApp adapter in the backend was directly passing unvalidated user-provided \`mediaUrl\` inputs to \`MessageMedia.fromUrl()\`. This function makes an HTTP request to download the file. Without validation, an attacker could supply a URL pointing to localhost (e.g., \`http://127.0.0.1\`) or internal/private IP ranges (e.g., AWS metadata service at \`169.254.169.254\`), potentially accessing internal services or scanning the internal network.
**Learning:** Functions that fetch remote resources based on user input are primary vectors for SSRF. It's crucial to validate that the requested URL belongs to expected protocols and does not resolve to internal network addresses.
**Prevention:** Implement rigorous URL validation. Parse the URL, assert that the protocol is \`http:\` or \`https:\`, and explicitly deny requests where the hostname is \`localhost\`, \`127.0.0.1\`, \`0.0.0.0\`, or any internal subnet (like \`10.x.x.x\` or \`192.168.x.x\`).
